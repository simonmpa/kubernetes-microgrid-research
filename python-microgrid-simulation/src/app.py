...
def remove_aggregated_microgrids(gridnames: list[str]):
    ...
    # Remove 'XX00' only if there are other regions with same code.
    result = []
...
def generate_grid_modules(c_names: list, co2: dict, final_step: int, electricity_price: dict):
    ...
    for name in c_names:
        ...
        co2_value = co2.get(lookup_code, 0.6426241177140256) # Use highest score if not found (Poland)
